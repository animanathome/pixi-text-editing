...
describe('fontAtlasText', () => {
    it('can render latin text', async () => {
        ...
        const fontUrl = 'http://localhost:8000/resources/MontserratBold.ttf'
        fontLoader.sourceUrl = fontUrl;
        await fontLoader.load();

        // https://freetype.org/freetype2/docs/glyphs/glyphs-3.html
        // https://www.npmjs.com/package/fontkit-next
        // console.log('ascent', fontLoader.font.ascent)
        // console.log('descent', fontLoader.font.descent)
        // console.log('lineGap', fontLoader.font.lineGap)
        console.log('----------------');
    ...
    describe('calculates accurate bounding box', () => {
        it('for the object', async() => {
            const fontLoader = new FontLoader();
            ...
            // Assert
            const {x, y, width, height} = text._calculateBounds();
            expect(x).to.equal(0);
            expect(y).to.equal(0);
            expect(width).to.equal(105.06);
            expect(height).to.equal(38.34);
        });

        it('for each line', async() => {

        });

        it('for each word', async() => {
